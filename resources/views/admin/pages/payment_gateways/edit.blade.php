...
@section('content')
    ...
    @endpush
    {!! Form::model($payment_gateway,['route'=>['admin.payment_gateways.update',$payment_gateway->id],'method'=>'PUT','files'=>true,'id'=>'payment_gateways_edit_form']) !!}
        <div class="container-fluid">
            <div class="row">
                <div class="col">
                    <h4 class="title p-4">
                        مدیریت درگاه های پرداخت - ویرایش
                    </h4>
                </div>
            </div>
            <div class="row">
                <div class="col">
                    <x-box title="ویرایش درگاه پرداخت" :buttons="$buttons">
                        <div class="row">
                            <div class="col">
                                <div class="form-group my-3">
                                    ...
                                    <select name="payment_system_id" id="payment_system_id" class="form-select form-select-sm" autocomplete="off">
                                        ...
                                        @foreach ($payment_systems as $item)
                                            <option value="{{$item->id}}" {{$item->id == $payment_gateway->payment_system_id ? 'selected' : ''}}>{{$item->title}}</option>
                                        @endforeach 
                                    </select>
                                </div>
                                ...
                                <div class="form-group my-3">
                                    <div class="form-check">
                                        {!! Form::checkbox('status', null, $payment_gateway->status,['class'=>'form-check-input']) !!}
    ...
    @push('script')
        <script>
            $('#payment_gateways_edit_form').validate({
                rules:{
                    payment_system_id:{
                        required: true
                    },
