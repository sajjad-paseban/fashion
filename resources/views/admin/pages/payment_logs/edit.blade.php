...
@section('content')
    ...
    @endpush
    {!! Form::model($payment_log,['route'=>['admin.payment_logs.update',$payment_log->id],'method'=>'PUT','id'=>'payment_logs_edit_form']) !!}
        <div class="container-fluid">
            <div class="row">
                <div class="col">
                    <h4 class="title p-4">
                        مدیریت گزارش پرداختی ها - ویرایش
                    </h4>
                </div>
            </div>
            <div class="row">
                <div class="col">
                    <x-box title="ویرایش گزارش پرداخت" :buttons="$buttons">
                        <div class="row">
                            <div class="col">
                                <div class="form-group my-3">
                                    ...
                                    <select name="post_id" id="post_id" class="form-select form-select-sm" autocomplete="off">
                                        ...
                                        @foreach ($posts as $item)
                                            <option value="{{$item->id}}" {{$item->id == $payment_log->post_id ? 'selected' : ''}}>{{$item->title}}</option>
                                        @endforeach 
                                    </select>
                                </div>
                                <div class="form-group my-3">
                                    ...
                                    <select name="user_id" id="user_id" class="form-select form-select-sm" autocomplete="off">
                                        ...
                                        @foreach ($users as $item)
                                            <option value="{{$item->id}}" {{$item->id == $payment_log->user_id ? 'selected' : ''}}>{{$item->name ." - ". ($item->email ? $item->email : $item->phone)}}</option>
                                        @endforeach 
                                    </select>
                                </div>
                                <div class="form-group my-3">
                                ...
                                <div class="form-group my-3">
                                    <div class="form-check">
                                        {!! Form::checkbox('status', null, $payment_log->status,['class'=>'form-check-input']) !!}
    ...
    @push('script')
        <script>
            $('#payment_logs_edit_form').validate({
                rules:{
                    post_id:{
                        required: true
                    },
